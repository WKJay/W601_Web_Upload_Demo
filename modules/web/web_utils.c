...
static int check_create_path(const char *path) {
    DIR *dirp = NULL;
    dirp = opendir(path);
    if (dirp) {
        closedir(dirp);
...
int relative_path_2_absolute(char *path_from, char *path_to, int to_length, char *absolute_prefix) {
    int max_length = 0;
...
int get_file_crc32(char *path, uint32_t *val) {
    ...
    do {
        // read data to block
        ret = read(fd, data_block, sizeof(data_block));
        if (ret < 0) {
            LOG_E("read file %s failed", path);
            ret = -1;
            goto exit;
        }
        // calculate block crc
        for (int i = 0; i < ret; i++) {
            crc32_value =
                crc32_tab[(crc32_value ^ data_block[i]) & 0xFF] ^ ((crc32_value >> 8) & 0x00FFFFFF);
        }
    } while (ret > 0);
    *val = crc32_value ^ 0xFFFFFFFF;
    ret = 0;
...
int create_file_by_path(char *path) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (path[i + 1] == '/') {
            ...
            fd = open(path_buff, O_CREAT | O_RDWR | O_TRUNC);
